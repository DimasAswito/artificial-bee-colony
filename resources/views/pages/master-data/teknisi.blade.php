...
@section('content')
    <x-common.page-breadcrumb pageTitle="Data Teknisi" />

    <div x-data="teknisiPageData()" x-init="init()" class="space-y-6">
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <!-- Card 1: Input Data Teknisi -->
            <div class="lg:col-span-2">
                <x-common.component-card title="Input Data Teknisi">
                    <form @submit.prevent="saveTeknisi">
                        <div class="space-y-4">
                            <!-- Nama Teknisi -->
                            <div>
                                ...
                                <input type="text" x-model="form.nama" placeholder="Masukkan Nama Teknisi"
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <template x-for="teknisi in paginatedData" :key="teknisi.id">
                                <tr>
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="getStatusClass(teknisi.status)" x-text="teknisi.status"></span>
                                    </td>
                                    <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-2">
                                            <button @click="openEditModal(teknisi)" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-400">
                                            ...
                                            <button @click="confirmDelete(teknisi.id)" class="text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400">
                                                <svg class="stroke-current" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
            <div class="flex justify-between gap-2 mb-4 sm:items-center">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Histori Data Teknisi</h3>
                </div>
            </div>
    ...
    <div x-show="editModalOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
        ...
        <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-800"
            ...
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Edit Teknisi</h3>
    ...
    <script>
        function teknisiPageData() {
            return {
                ...
                async fetchTeknisi() {
                    try {
                        const response = await fetch('{{ route('teknisi.data') }}');
                        const data = await response.json();
                ...
                get filteredData() {
                    ...
                    return this.teknisiData.filter(d => 
                ...
                openEditModal(teknisi) {
                    this.form = {
                        id: teknisi.id,
                        nama: teknisi.nama,
                        status: teknisi.status || 'Active'
                    };
                    this.isEditing = true;
                ...
                resetForm() {
                    this.form = { id: null, nama: '', status: 'Active' };
                ...
                async saveTeknisi() {
                    ...
                    const url = this.isEditing ? `/teknisi/${this.form.id}` : '{{ route('teknisi.store') }}';
                    ...
                    try {
                        ...
                        const successTitle = this.isEditing ? 'Data teknisi berhasil diperbarui' : 'Data teknisi berhasil ditambahkan';

                        await this.fetchTeknisi();
                ...
                confirmDelete(id) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                             try {
                                const response = await fetch(`/teknisi/${id}`, {
                                    method: 'DELETE',
                                    headers: {
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                                    }
                                ...
                                if (response.ok) {
                                    ...
                                    Toast.fire({
                                        icon: 'success',
                                        title: 'Data teknisi berhasil dihapus'
                                    });
                                } else {
